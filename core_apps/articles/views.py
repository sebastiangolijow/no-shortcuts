...
from .renderer import ArticleJSONRenderer, ArticlesJSONRenderer
...
class ArticleRetrieveUpdateDestroyView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        ArticleView.record_view(
            article=instance, user=self.request.user, viewer_ip=viewer_ip
        )
...
class ClapArticleView(generics.CreateAPIView, generics.DestroyAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if Clap.objects.filter(user=user, article=article).exists():
            return Response("You already clap this", status=status.HTTP_400_BAD_REQUEST)
        clap = Clap.objects.create(user=user, article=article)
        clap.save()
        return Response("Clap added", status=status.HTTP_201_CREATED)
    ...
    def delete(self, request, *args, **kwargs):
        ...
        clap.delete()
        return Response("Clap removed", status=status.HTTP_204_NO_CONTENT)
